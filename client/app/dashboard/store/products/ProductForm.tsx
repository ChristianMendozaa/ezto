...
export const ProductForm: React.FC<ProductFormProps> = ({ onSubmit, defaultValues, editMode }) => {
  const { t } = useLanguage();
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <Button>
          {editMode ? t("store.catalog.form.edit") : t("store.catalog.addProduct")}
        </Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>{editMode ? t("store.catalog.form.edit") : t("store.catalog.addProductTitle")}</DialogTitle>
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid gap-4 py-4">
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="barcode" className="text-right">{t("store.catalog.form.barcode")}</Label>
              <Input id="barcode" value={formData.barcode} onChange={handleInputChange} className="col-span-3" required />
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
          ...
          <DialogFooter>
            <Button type="submit" disabled={loading}>
              {editMode ? t("store.catalog.form.update") : t("store.catalog.form.save")}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
