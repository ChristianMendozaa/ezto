...
PORT = int(os.getenv("PORT", 8001))  # Ajusta según servicio
SERVICE_NAME = "shop-service"         # Cambia según cada microservicio
service_id = None
...
def register_service():
    consul_addr = os.getenv("CONSUL_ADDR", "http://consul:8500")
    service_id = f"shop-service-{uuid.uuid4()}"
    payload = {
        "ID": service_id,
        "Name": "shop-service",
        ...
        "Check": {
            "HTTP": f"http://{socket.gethostbyname(socket.gethostname())}:{os.getenv('PORT',8001)}/health",
            "Interval": "10s",
            "Timeout": "5s",
